...
def features(image_gray,image):
    ...
    # # erosion = cv2.erode(image_dilated,kernel,iterations = 1)    #used in Good Features to track
...
def Testudo(image,testudo_img,corner_points,desired_tag_corner):
...
# out = cv2.VideoWriter('Dijkstra_Video.mp4',cv2.VideoWriter_fourcc(*'DIVX'), 20, frameSize)
...
while(True):
    ...
    if corner_points is not None:
        ...
        image = Testudo(image,testudo_img,corner_points,desired_tag_corner)
